...
function handleSave() {
    try {
        ...
        // Body format: data=<URL-encoded JSON string>
        var eqIdx = body.indexOf("=");
        var raw = eqIdx > 0 ? decodeURIComponent(body.substring(eqIdx + 1)) : body;
...
'<p class="hint" style="margin-top:12px">保存后，cron 任务通过 #auto 自动读取</p>' +
'<form method="POST" action="/save" id="sf" style="display:none"><input type="hidden" name="data" id="sd"></form>' +
...
var isSave = url.indexOf("cfui.com/save") >= 0 && method === "POST";
